...
const Comment = ({ isOpen, isClose }) => {
  ...
  return (
    <dialog id="post_modal" className={`modal modal-bottom sm:modal-middle ${isOpen ? "modal-open" : ""}`}>
      <div className="modal-box relative bg-white p-0 flex flex-col max-h-[90vh]">
        {/* Static Header */}
        <div className="sticky top-0 bg-white z-20 p-4 border-b border-gray-200 flex-none">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold">Comments</h3>
            <button onClick={isClose} className="btn btn-sm btn-circle btn-ghost">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
          ...
          <div className="px-4 pb-4">
            ...
            {singlePost.comments?.map((com) => (
              ...
              >
                ...
                {com.userId?.profile_pic ? (
                  <img
                    src={com.userId.profile_pic || "/placeholder.svg"}
                    alt="Profile"
                ...
                <div className="flex-1">
                  <div className="bg-white p-3 rounded-xl">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-gray-500">{new Date(com.createdAt).toLocaleDateString()}</span>
                    </div>
        ...
        {/* Static Input Bottom */}
        <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 flex-none">
          ...
          {error && <div className="text-error text-sm mt-2">Failed to post comment: {error}</div>}
        </div>
      </div>
    </dialog>
  )
